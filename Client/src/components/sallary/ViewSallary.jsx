...
function ViewSallary() {
    ...
    const fetchSallaries  = async () =>{
        try {
            const res  = await axios.get(`https://ems-employee-management-system-api.vercel.app/api/sallary/${id}/${user.role}` , {headers : {'Authorization' : `Bearer ${localStorage.getItem("token")}`}})
            if(res.data.success){
